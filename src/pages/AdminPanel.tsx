        ...
        <div className="container mx-auto px-4 py-6 max-w-7xl">
            ...
            <div className="bg-white rounded-lg shadow overflow-hidden mb-6">
                {filteredEvents.length === 0 ? (
                    <div className="text-gray-500 text-center py-8">Nenhum evento recente.</div>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead className="bg-gray-50">
                                <tr>
                                    <th className="text-left p-3 font-semibold text-gray-600">Evento</th>
                                    ...
                                    const badgeColor =
                                        ev.event === 'whatsapp_enviado' ? 'bg-green-100 text-green-700' :
                                            ev.event === 'consulta_iniciada' ? 'bg-blue-100 text-blue-700' :
                                                ev.event === 'consulta_sucesso' ? 'bg-emerald-100 text-emerald-700' :
                                                    ev.event === 'consulta_vazia' ? 'bg-red-100 text-red-700' :
                                                        ev.event === 'carrinho_adicionado' ? 'bg-purple-100 text-purple-700' :
                                                            ev.event === 'orcamento_visualizado' ? 'bg-amber-100 text-amber-700' :
                                                                ev.event === 'abandono' ? 'bg-rose-100 text-rose-700' :
                                                                    'bg-gray-100 text-gray-700';

                                    // Pula eventos de teste
                                    if (ev.event === 'teste_final' || ev.event === 'diagnostico' || ev.event === 'teste') {
                                        return null;
                                    }
                ...
                {filteredEvents.filter(ev => ev.event !== 'teste_final' && ev.event !== 'diagnostico' && ev.event !== 'teste').length === 0 && (
                    <div className="text-gray-500 text-center py-4">Nenhum evento válido.</div>
                )}
            </div>
